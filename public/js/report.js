...
show_service_btn.addEventListener("click",()=>{
  services.classList.toggle("d-none")
...
report_form.addEventListener("submit", async (event) => {
  ...
  const { data: saha } = await axios.post("/report/saha", { start_at: start, end_at: end })
  for (let i = 0; i < saha.length; i++) {
    ...
    var updatedTime23HourFormat = (hours < 10 ? '0' : '') + hours + ':' + (minutes < 10 ? '0' : '') + minutes;

    // Combine the original date with the updated time in the original format
    var updatedDateTimeString = firstDate.replace(/\d{2}:\d{2}:\d{2}$/, updatedTime23HourFormat);
  ...
  for (let i = 0; i < workdays.length; i++) {
    const { data } = await axios.post("/report/services", { report_at: workdays[i] });
...
// return
  ...
  if (positionOfFirstSample < 400) {
    sorted_sub_rows.unshift({ row: 1, piling_code: 'CR 1-1-1', sampling_at: start_at, d: 60, sub_row: '1' })
  } else {
    sorted_sub_rows.unshift({ row: 1, piling_code: 'CR 1-1-1', sampling_at: start_at, d: 600, sub_row: '1' })
  }
  ...
  for (let i = 0; i < sorted_sub_rows.length; i++) {
    if (sorted_sub_rows[i + 1]) {
      intervals[`${i}-${i + 1}`] = { piling_code: sorted_sub_rows[i + 1].piling_code, row: sorted_sub_rows[i + 1].row, sub_row: sorted_sub_rows[i + 1].sub_row, d1: sorted_sub_rows[i].d, d2: sorted_sub_rows[i + 1].d, time1: sorted_sub_rows[i].sampling_at, time2: sorted_sub_rows[i + 1].sampling_at, services: [] };
    }
  }
  ...
  for (let i = 0; i < intervalsArray.length; i++) {
    ...
    if (i == 0) {
      ...
      if (row != intervalsArray[i - 1].row || sub_row != intervalsArray[i - 1].sub_row) {
        changeRow = true
  ...
  const divided = {};
  // devided is devide all service between 60-80 , ....
  ...
  for (const item in divided) {
    ...
    for (let i = 0; i < finalArrayOfSubrows.length; i++) {
      if (Number(finalArrayOfSubrows[i].position) > min && Number(finalArrayOfSubrows[i].position) < max) {
